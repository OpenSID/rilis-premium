  ...
  (function() {
    ...
    window.onload = function() {
      ...
      const generatePopupDusun = function(data){
        ...
        const _wilayah = { level : 1, key: 'dusun',  sebutan : "{{ ucwords(setting('sebutan_kepala_dusun')) }}", div_parent : 'isi_popup_dusun' }
      ...
      const generatePopupRw = function(data){
        ...
        const _wilayah = { level : 2, key: 'rw',  sebutan : "RW", div_parent : 'isi_popup_rw' }
      ...
      const generatePopupRt = function(data){
        ...
        const _title = `{{ ucwords(setting('sebutan_dusun')) }}`
        const _wilayah = { level : 3, key: 'rt',  sebutan : "RT", div_parent : 'isi_popup_rt' }
      ...
      const generatePopupElement = function(data, pengaturan, gis, _link, _title, _wilayah){        
        ...
        for(let _key in gis){
          ...
          if (pengaturan.includes('Aparatur Desa')){
            _contentHTML += `<p><a href="{{ci_route("load_aparatur_wilayah")}}/${gis[_key]['id_kepala']}/${_wilayah['level']}" class="btn btn-social bg-navy btn-sm btn-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" data-title="{{ ucwords(setting('sebutan_kepala_dusun')) . ' ' . $dusun['dusun'] }}" data-remote="false" data-toggle="modal" data-target="#modalKecil"><i class="fa fa-user"></i>&nbsp;&nbsp;${_wilayah['sebutan']}&nbsp;&nbsp;</a></p>`
          }
          _elemenHTML = `
