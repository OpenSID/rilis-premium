		...
		<div class="content_middle_middle">
			<div id="aparatur_desa" class="cycle-slideshow" data-cycle-pause-on-hover=true data-cycle-fx=scrollHorz
				...
				@php
					...
					if ($data['kehadiran'] == 1) {
						$desc .= "<span class='label label-success'>" . ($data['status_kehadiran'] == 'hadir' ? 'Hadir' : '') . "</span>"
				...
				@endphp
				<img data-src="{{ $data['foto'] }}" src="{{ asset('images/img-loader.gif') }}" class="yall_lazy"
					data-cycle-title="{{ $desc }}" data-cycle-desc="{{ $data['jabatan'] }}">
				@endforeach
			</div>
		</div>
	</div>
