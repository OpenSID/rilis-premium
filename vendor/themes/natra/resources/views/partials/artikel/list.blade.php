    ...
    <li>
      <div class="catgimg2_container2">
        ...
        <a href="{{ $post->url_slug }}" title="Baca Selengkapnya" style="font-weight:bold">
          ...
          <img data-src="{{ AmbilFotoArtikel($post['gambar'], 'sedang') }}" src="{{ asset('images/img-loader.gif') }}"
            width="300" class="yall_lazy img-fluid img-thumbnail hidden-sm hidden-xs"
            style="float:left; margin:0 8px 4px 0;" alt="{{ $post['judul'] }}" />
          <img data-src="{{ AmbilFotoArtikel($post['gambar'], 'sedang') }}" src="{{ asset('images/img-loader.gif') }}"
            width="100%" class="yall_lazy img-fluid img-thumbnail hidden-lg hidden-md"
            style="float:left; margin:0 8px 4px 0;" alt="{{ $post['judul'] }}" />
          @else
          <img src="{{ theme_asset('images/noimage.png') }}" width="300px"
            class="img-fluid img-thumbnail hidden-sm hidden-xs" style="float:left; margin:0 8px 4px 0;"
            alt="{{ $post['judul'] }}" />
          <img src="{{ theme_asset('images/noimage.png') }}" width="100%"
            class="img-fluid img-thumbnail hidden-lg hidden-md" style="float:left; margin:0 8px 4px 0;"
            alt="{{ $post['judul'] }}" />
          @endif
        </a>
