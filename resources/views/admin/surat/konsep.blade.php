...
@php filemanager_authorize('cetak-surat'); @endphp
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="{{ $surat->formatSurat->url_surat ?? $surat->url_surat }}">
                        <div class="box-body">
                            ...
                            <div class="form-group">
                                <textarea id="editor" name="isi_surat" data-filemanager='<?= json_encode(['external_filemanager_path' => filemanager_base_url(), 'filemanager_title' => 'File Manager']) ?>' data-salintemplate="isi" class="form-control input-sm editor required">{{ $isi_surat }}</textarea>
                            </div>
                        </div>
                    </div>
                    @foreach ($lampiran as $kode => $isiLampiran)
                        <div class="tab-pane" id="{{ $loop->index }}">
                            <div class="box-body">
                                <div class="form-group">
                                    <textarea name="isi_lampiran[]" data-filemanager='<?= json_encode(['external_filemanager_path' => filemanager_base_url(), 'filemanager_title' => 'File Manager']) ?>'
                            ...
                            try {
                                ...
                                if (data.saved_as_konsep) {
                                    Swal.fire({
                                        ...
                                        icon: 'warning',
                                        text: data.message,
                                        confirmButtonText: 'Ke Arsip Layanan',
                                        allowOutsideClick: false
                                    }).then((result) => {
                                        window.location.href = data.redirect || '{{ ci_route("keluar/masuk") }}';
                                    });
                                } else {
                                    Swal.fire({
                                        title: 'Nomor Surat Bentrok',
