...
@section('step')
    ...
    <div class="flex flex-wrap border border-gray-200 text-gray-800 rounded-md mb-4 divide-y divider-gray-200">
        @foreach ($ci->config->item('server') as $check)
            @php
                $devBypassed = isset($check['dev_check']) && ! app()->environment('production') && ! $check['dev_check']();
                $passed = $devBypassed ? false : $check['check']();
            @endphp
            <div class="w-full px-4 py-2 text-gray-800">
                ...
                @endif
                @if ($devBypassed)
                    <small class="ml-2 text-yellow-600 font-medium">— dilewati di lingkungan pengembangan</small>
                @endif
                @if (! $passed && ! empty($check['peringatan']))
                    <small class="ml-2 text-yellow-600 font-medium">— peringatan, instalasi tetap dapat dilanjutkan</small>
                @endif
