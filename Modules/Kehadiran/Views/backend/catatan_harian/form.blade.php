        ...
        <div class="form-group">
            ...
            <div class="col-sm-3">
                <input type="date" name="tanggal" class="form-control input-sm"
                    ...
                    max="{{ date('Y-m-d') }}" {{ $isView ? 'disabled' : '' }}>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <textarea name="uraian_kegiatan" class="form-control input-sm" rows="4"
                    placeholder="Deskripsi kegiatan yang dilakukan" {{ $isView ? 'disabled' : '' }}>{{ old('uraian_kegiatan', $catatan_harian?->uraian_kegiatan ?? '') }}</textarea>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <textarea name="hasil_diharapkan" class="form-control input-sm" rows="3"
                    placeholder="Hasil yang diharapkan (opsional)" {{ $isView ? 'disabled' : '' }}>{{ old('hasil_diharapkan', $catatan_harian?->hasil_diharapkan ?? '') }}</textarea>
            </div>
        </div>
                    ...
                    <div class="panel-body" style="padding: 15px;">
                        ...
                        <div id="file-inputs-container" class="row">
                            ...
                            @if ($catatan_harian && $catatan_harian->fotos->isNotEmpty())
                                @foreach ($catatan_harian->fotos->take(5) as $index => $foto)
                                    <div class="file-input-group col-sm-4 col-xs-6" style="margin-bottom: 15px;">
                                        <div
                                            style="border: 2px solid #17a2b8; border-radius: 6px; padding: 12px; background-color: #f9f9f9; text-align: center; position: relative; min-height: 280px; display: flex; flex-direction: column; justify-content: space-between;">
                                            <div
                                                style="font-weight: 600; color: #333; font-size: 14px; margin-bottom: 8px;">
                                                Foto {{ $index + 1 }}</div>

                                            {{-- Preview Area --}}
                                            <div class="photo-preview"
                                                style="flex-grow: 1; display: flex; align-items: center; justify-content: center; margin-bottom: 8px; min-height: 180px; overflow: hidden; position: relative;">
                                                <img class="original-image" src="{{ base_url($foto->file_path) }}"
                                                    style="max-width: 100%; max-height: 180px; object-fit: contain; cursor: pointer;"
                                                    onclick="showPhotoWithNumber(this)">
                                            </div>
                                            ...
                                            @if ($isView)
                                                ...
                                                <div class="button-group" style="display: flex; gap: 6px;">
                                                    <button type="button" class="btn btn-sm btn-info"
                                                        style="flex: 1; background-color: #17a2b8; color: white; border-color: #17a2b8;"
                                                        onclick="showPhotoWithNumber(this.parentElement.parentElement.querySelector('.original-image'))">
                                            ...
                                            @else
                                                ...
                                                {{-- File Input (Hidden) --}}
                                                <input type="file" class="file-input existing-file-input"
                                                    name="fotos_replace[]" accept=".jpg,.jpeg,.png" style="display: none;">

                                                <div class="button-group" style="display: flex; gap: 6px;">
                                                    <button type="button" class="btn btn-sm file-browser"
                                                        style="flex: 1; background-color: #17a2b8; color: white; border-color: #17a2b8;">
                                                        <i class="fa fa-pencil"></i> Ganti
                                                    </button>
                                                    <button type="button" class="btn btn-sm btn-danger delete-foto"
                                                        data-foto-id="{{ $foto->uuid }}"
                            ...
                            @if (!$isView && (!$catatan_harian || $catatan_harian->fotos->isEmpty()))
                                <div class="file-input-group col-sm-4 col-xs-6" style="margin-bottom: 15px;">
                                    <div
                                        style="border: 2px dashed #17a2b8; border-radius: 6px; padding: 12px; background-color: #f9f9f9; text-align: center; position: relative; min-height: 280px; display: flex; flex-direction: column; justify-content: space-between;">
                                        ...
                                        <div class="photo-preview"
                                            ...
                                            <div class="placeholder-text"
                                                style="text-align: center; color: #999; z-index: 1;">
                                                <i class="fa fa-camera"
                                                    style="font-size: 48px; margin-bottom: 5px; display: block;"></i>
                                                <small style="font-size: 13px;">Pilih foto</small>
                                            </div>
                                            <img class="preview-image"
                                                style="position: absolute; max-width: 100%; max-height: 180px; object-fit: contain; display: none; cursor: pointer;"
                                                onclick="showPhotoWithNumber(this)">
                                        </div>

                                        {{-- Hidden File Input --}}
                                        <input type="file" class="hidden file-input" name="fotos[]"
                                            accept=".jpg,.jpeg,.png">
                                        ...
                                        <div style="display: flex; gap: 6px;">
                                            ...
                                            <button type="button" class="btn btn-sm btn-danger btn-remove-file" disabled
                                                style="flex: 0 0 auto; padding: 6px 10px;">
                                                <i class="fa fa-trash"></i>
                                            </button>
                                        </div>
        ...
        function showImageSweetAlert(imageSrc, title) {
            Swal.fire({
                ...
                html: '<img src="' + imageSrc +
                    '" style="max-width: 90%; max-height: 500px; object-fit: contain; display: block; margin: 0 auto;">',
